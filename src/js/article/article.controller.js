...
class ArticleCtrl {
  constructor(article, User, Comments, $sce, $rootScope) {
    ...
    this.article.body = $sce.trustAsHtml(marked(this.article.body, { sanitise: true }));
  ...
  addComment () {
    ...
    this._Comments
      .add(this.article.slug, this.commentForm.body)
      .then(
        comment => {
  ...
  deleteComment (commentId, index) {
    this._Comments
      .destroy(commentId, this.article.slug)
      .then(
